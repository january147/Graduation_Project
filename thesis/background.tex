...
\subsection{Xposed}
 Xposed框架\upcite{xposed} 是由rovo89开发的一款针对Android平台的动态劫持工具，是一款可以在不修改APK的情况下影响程序运行, 修改系统的框架服务\juhao Xposed通过替换/system/bin/app\_process 程序控制Zygote进程，使app\_process 在启动过程中加载XposedBridge.jar这个jar包，从而完成对Zygote进程及其创建的Dalvik虚拟机的劫持\juhao Reaper\upcite{reaper}\dunhao Inspeckage\upcite{inspeckage}\dunhao InsightDroid\upcite{chinese1} 文献\cite{chinese2}中的系统使用该技术实现了动态监控\juhao 

\subsection{Valgrind}
Valgrind\upcite{valgrind}是一个用于构建动态分析工具的代码追踪框架\juhao Malton\upcite{malton}\dunhao PackerGrind\upcite{packergrind}使用了该技术来实现动态监控\juhao 
