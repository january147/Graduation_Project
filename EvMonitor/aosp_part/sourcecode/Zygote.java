...
public final class Zygote {
    ...
    */
    ...
    public static void logAppStartAndSetTarget_em(String process_name, String app_name ){
        ...
        String target_type = SystemProperties.get("em.target_type");
        Log.d("EvMonitor", "app started--ProcessName:[" + process_name + "]--PackageName:[" + app_name + "]");
        // monitor by process name or app_name, according to sytem property [em.target_type]. the default setting is monitoring by process name.
        if (target_type == null || target_type.equals(em_PROCESS)) {
            String target_process = SystemProperties.get("em.target_process");
            if (process_name.equals(target_process)) {
                enable_monitor = true;
            }
    
        } else if (target_type.equals(em_APP)) {
            String target_app = SystemProperties.get("em.target_app");
            if (app_name.equals(target_app)) {
                enable_monitor = true;
            }
        }
