...
static inline JValue Execute(
  ...
  ArtMethod* this_method = shadow_frame.GetMethod();
  this_method->log_em("IE");
  // end EvMonitor
    ...
    if (UNLIKELY(instrumentation->HasMethodEntryListeners())) {
      ...
      if (UNLIKELY(self->IsExceptionPending())) {
        instrumentation->MethodUnwindEvent(self,
                                           ...
                                           method,
                                           0);
        // EvMonitor method out
        this_method->log_em("OE");
        // end EvMonitor
        return JValue();
      }
    }
  ...
  if (LIKELY(method->SkipAccessChecks())) {
    ...
    if (kInterpreterImplKind == kMterpImplKind) {
      if (transaction_active) {
        ...
        result = ExecuteSwitchImpl<false, true>(self, code_item, shadow_frame, result_register,
                                              false);
        // EvMonitor method out
        this_method->log_em("OE");
        // end
        ...
        result = ExecuteSwitchImpl<false, false>(self, code_item, shadow_frame, result_register,
                                               false);
        // EvMonitor
        this_method->log_em("OE");
        // end
    ...
    if (kInterpreterImplKind == kMterpImplKind) {
      ...
      if (transaction_active) {
        result = ExecuteSwitchImpl<true, true>(self, code_item, shadow_frame, result_register,
                                             false);
        // EvMonitor
        this_method->log_em("OE");
        // end
        ...
        result = ExecuteSwitchImpl<true, false>(self, code_item, shadow_frame, result_register,
                                              false);
        // EvMonitor
        this_method->log_em("OE");
        // end
