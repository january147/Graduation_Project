...
DexFile::DexFile(const uint8_t* base,
  ...
  // EvMonitor--monitor dexfile parsing
  if (em.monitor_enabled) {
    __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "dexFileParsed--file address:[%p]--file size:[%zu]", base, size);
  }
  if (em.dex_dump_enabled) {
    em.dumpDex(base, size);
  }
  // end EvMonitor
