...
static void com_android_internal_os_Zygote_nativeEnableMonitor(JNIEnv* env, jclass, jint jpid){
  ...
  std::string app;
  int pid = jpid;
  art::setMonitorPid(pid);
  art::em.enableMonitor(true);
  property_get("target_app", target_app, "0");
  if (target_app[0] != '0'){
    app = target_app;
  }
  art::em.setTargetApp(app);
  art::em.enableEmbedLog(true);
  art::em.enableDexDump(true);
  __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "native interface seted up, pid[%d]", pid);
}
