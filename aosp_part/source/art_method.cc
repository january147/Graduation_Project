...
void EvMonitor::enableMonitor(bool enable) {
  ...
  if (enable) {
    if (setLogFilePath()) {
      ...
      if ((void*)log_data == (void*)-1) {
        __android_log_print(ANDROID_LOG_ERROR, "EvMonitor", "can't allocate log buff, error[%s]", strerror(errno));
        return false;
      }
      log_base = log_data;
      embeded_log_enabled = true;
#ifdef EM_DEBUG
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "embeded log enabled, log buff address[%p]", log_base);
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "log buff size[%d]", LOG_MAX);
#endif
    }else{
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "can't get log file dir");
      return false;
    }
  }else{
...
bool EvMonitor::enableDexDump(bool enable) {
  if (enable) {
    if (app_root.length() != 0) {
      file_dump_dir = app_root + "/dumped_file";
      if (access(file_dump_dir.c_str(), F_OK) < 0) {
        if(mkdir(file_dump_dir.c_str(), 0755) < 0) {
          __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "can't create dumped file dir, error[%s]", strerror(errno));
          return false;
        }
...
bool EvMonitor::setLogFilePath() {
  ...
  log_dir_root = app_root + "/log_dir";
  if (access(log_dir_root.c_str(), F_OK) < 0) {
    if (mkdir(log_dir_root.c_str(), 0755) < 0) {
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "can't create root log dir[%s], error[%s]", log_dir_root.c_str(), strerror(errno));
      return false;
    }
  }
  current_log_dir = log_dir_root + "/" + getTime();
  if (access(current_log_dir.c_str(), F_OK) < 0) {
    if (mkdir(current_log_dir.c_str(), 0755) < 0) {
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "can't create log dir[%s], error[%s]", current_log_dir.c_str(), strerror(errno));
      return false;
    }
  }
  __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "log file dir[%s]", current_log_dir.c_str());
  return true;
}

void EvMonitor::writeLog() {
  std::stringstream file_name_builder;
  if(log_file_amount < LOG_FILE_MAX){
    ...
    if(dumpFile(log_base, LOG_MAX - log_spare_space, file_name_builder.str())) {
      ...
      __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "can't save log file, error[%s]", strerror(errno));
    }
  } else {
    __android_log_print(ANDROID_LOG_DEBUG, "EvMonitor", "log file reached max size");
  }
}
...
void EvMonitor::dumpDex(const void* base, long size) {
  ...
  if (dex_dump_enabled) {
    time(&t);
    file_name_builder << file_dump_dir << "/" << getRandomStr(6) << "_" << t;
