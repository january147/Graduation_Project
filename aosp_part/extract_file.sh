...
zygote_connection_java=$com_android_internal_os_java/ZygoteConnection.java

activity_thread_java=$android_app_java/ActivityThread.java
zygote_cpp=$fcore_jni/com_android_internal_os_Zygote.cpp

# cp to current dir
files=($art_method_cc $art_method_h $dex_file_cc $Evmonitor_h $zygote_java $zygote_connection_java $activity_thread_java $zygote_cpp)
for file in ${files[*]}
do
    cp $file ./source/
done
